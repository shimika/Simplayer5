...
namespace ShimiKore {
		...
		private void BwUpdate_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e) {
			...
			try {
				...
				if (UpdateComplete != null) {
					...
					UpdateComplete(this, new UpdateCompleteArgs(true, null));
				}
			}
			catch (Exception ex) {
				if (UpdateComplete != null) {
					UpdateComplete(this, new UpdateCompleteArgs(false, null));
				}
			}
		}
	}
	...
	public class UpdateCompleteArgs : EventArgs {
		...
		public Dictionary<string, string> DictParams;

		public UpdateCompleteArgs(bool v, Dictionary<string, string> d) {
			this.Complete = v;
			this.DictParams = d;
		}
	}
